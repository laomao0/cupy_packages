...
class Encode(torch.nn.Module):
    ...
    def __init__(self, intIns:typing.List[int], intOuts:typing.List[int], strHor:str, strVer:str, objScratch:typing.Dict[str, typing.List[int]]):
        ...
        self.intIns = intIns.copy()# 000000
        ...
        for intRow in range(self.intRows):
            netHor = torch.nn.Identity() # return inputs
...
class Network(torch.nn.Module):
    def __init__(self):
        ...
        )) # Decode[0,64,128,256,512] [0,64,128,256,512]
        ...
        try:
            ...
            print('load failed~')
        # self.load_state_dict(torch.load('/DATA/wangshen_data/CODES/revisiting-sepconv/network-paper.pytorch'))
    # end
    ...
    def forward(self, tenSeq):
        ...
        tenVerone = self.netVerone(tenOut) # N 51 H W
...
def estimate(tenOne, tenTwo):
    ...
    tenPreprocessedOne = tenOne.cuda().view(1, 3, intHeight, intWidth) # NCHW
...
if __name__ == '__main__':
    if arguments_strOut.split('.')[-1] in [ 'bmp', 'jpg', 'jpeg', 'png' ]:
        tenOne = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(PIL.Image.open(arguments_strOne))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))) # CHW
        tenTwo = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(PIL.Image.open(arguments_strTwo))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))) # CHW
